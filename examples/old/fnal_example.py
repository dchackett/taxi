...
local_util.use_shared_binary_paths()
...
seed_stream = dispatch_tools.make_hmc_job_stream(Ns=4, Nt=4, beta=7.75, k4=0.128, k6=0.1285, # Physical parameters to run
                                               ...
                                               req_time=180, # Estimate max 3 minutes to run one HMC task
...
# Currently, no binaries for these jobs on FNAL
aux_job_pool = []
...
d = dispatch_tools.dispatch(
    ...
    taxi_time=23.5*60*60, # Maximum time in seconds a taxi can run until it needs to resubmit itself (here, 23.5h)
