...
pre_existing_gauge_files = ['./sample_data/GaugeSU4_4_4_7.75_0.128_0.1285_1_0',]

## Auxiliary jobs: Flow, spectroscopy
aux_job_pool = []

# Spectroscopy helper functions -- provide a list of saved gauge filenames, and it will make a spectroscopy job for each one
aux_job_pool += dispatch_tools.spectro_jobs_for_gaugefiles(pre_existing_gauge_files, # Parses relevant physical parameters from saved gauge filenames
...
aux_job_pool += dispatch_tools.flow_jobs_for_gaugefiles(pre_existing_gauge_files,     # Takes relevant physical parameters from HMC jobs
...
# Flow helper function -- provide a list of HMC jobs, and it will make a gradient flow job for each one (after and including start_count)
aux_job_pool += dispatch_tools.hrpl_jobs_for_gaugefiles(pre_existing_gauge_files)     # Takes relevant physical parameters from HMC jobs
...
d = dispatch_tools.dispatch(
    ...
    # None of these jobs will have any dependencies; we don't want to launch N-thousand taxis, so set a limit
    max_taxis=10)
