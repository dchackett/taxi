...
parser.add_argument('--k4',       type=float, required=True,  help='Kappa parameter for fundamental fermions.')
parser.add_argument('--k6',       type=float, default=0,      help='(IGNORED, left for convenience) Kappa parameter for 2-index antisymmetric fermions.')
...
""".format(k4=k4, shift=shift, maxcgpf=maxcgpf, cgtol=cgtol)

    if nsteps2 is not None:
        # Second pseudofermion level for Hasenbuch
        output_str += \
"""
pfid twom_f        # a unique name, for identification in the outfile
