...
parser = argparse.ArgumentParser(description="Run a Hybrid Monte Carlo simulation in SU(4) with fundamental fermions.")
...
parser.add_argument('--kG',       type=float, required=True,  help='Kappa parameter for adjoint fermions.')
parser.add_argument('--cswG',     type=float, default=1.,     help='Clover coefficient, 1 by default.')
parser.add_argument('--gammarat', type=float, default=None,   help='NDS action parameter. beta/gamma = gammarat, so gamma = beta/gammarat.')
...
def build_hmc_input_string(Ns, Nt, beta, gammarat, kG, cswG,
...
## gauge action params
beta {beta}
""".format(Ns=Ns, Nt=Nt, seed=seed, beta=beta

    if gammarat is not None:
        output_str += "gammarat {gammarat}\n".format(gammarat=gammarat)
...
## dynamical fermions
nkappas 1""".format(maxcgobs=maxcgobs, cgtol=cgtol)
...
""" 
## fermion actions
kpid kap_g         # identifier
kappa {kG}         # hopping parameter
csw {cswG}           # clover coefficient
#mu 0.0            # chemical potential
nf 2               # how many Dirac flavors
irrep adjt
    
## pf actions
pfid onem_g        # a unique name, for identification in the outfile
type onemass       # pseudofermion action types: onemass twomass rhmc
kpid kap_g         # identifier for the fermion action
...
""".format(kG=kG, cswG=cswG, shift=shift, maxcgpf=maxcgpf, cgtol=cgtol)

    if nsteps2 is not None:
        # Second pseudofermion level for Hasenbuch
        output_str += \
"""
pfid twom_g        # a unique name, for identification in the outfile
