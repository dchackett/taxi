...
def taxi_launcher(taxi_name, taxi_forest, home_dir, taxi_dir, taxi_time, taxi_nodes, taxi_shell_script):   
    ...
    summit_time_str = "{hours:02d}:{minutes:02d}:{seconds:02d}".format(hours=hours, minutes=minutes, seconds=seconds)
    
    ## Arguments to SLURM
    taxi_call  = "sbatch "
    taxi_call += " --nodes {taxi_nodes} --job-name {taxi_name} ".format(taxi_nodes=taxi_nodes, taxi_name=taxi_name)
    taxi_call += " --time {time_str} ".format(time_str=summit_time_str)

    # No special queues on Summit...yet.
#    if taxi_nodes == 1:
